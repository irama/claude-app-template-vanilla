...
- **Alternative runner — self-hosted n8n** (when GitHub Actions minutes are constrained): fold the dump into the existing keepalive workflow, but **stream** `pg_dump | aws s3 cp -` straight to R2 via an Execute Command node — NEVER buffer the dump in an n8n node (that OOM'd the VPS once and forced the move to Actions). A daily backup also keeps a free-tier DB from pausing (keepalive + backup = one job), but keep the lightweight keepalive query as the guaranteed floor so a failed dump never lets a project pause.
