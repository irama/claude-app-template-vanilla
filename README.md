...
Strict TypeScript (no `any`) · tests mandatory alongside new code · Zod on all API inputs · conventional commits · SQL/migrations live under `data/` (never top-level `supabase/` or `bigquery/`). Your own user-level `CLAUDE.md` carries anything that applies across all your projects; this file carries only what is true of this one.
